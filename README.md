...
- **Prices prefer Cardmarket (EUR) over TCGplayer (USD)**, falling back to
  TCGplayer only for a card with no Cardmarket data. Cardmarket is the
  reference UK/EU collectors and traders actually use, unlike the US-centric
  TCGplayer number the app originally (and wrongly) showed with a hardcoded
  £ and no conversion — since fixed to always label whichever currency the
  price actually is (`app/src/format.ts`), rather than fake-converting with
  a hardcoded exchange rate that would just go stale. **Unverified**: how
