...
ratingsRouter.post("/trades/:tradeId", (req: AuthedRequest, res) => {
  const trade = prepare(`SELECT * FROM trades WHERE id = ?`).get(req.params.tradeId) as TradeRow | undefined;
  if (!trade) return res.status(404).json({ error: "Not found" });
  if (!trade.completed_at) return res.status(400).json({ error: "Trade isn't marked complete by both parties yet" });

  const match = prepare(`SELECT * FROM matches WHERE id = ?`).get(trade.match_id) as MatchRow;
  ...
  try {
    const info = prepare(`INSERT INTO ratings (trade_id, rater_id, ratee_id, stars, review) VALUES (?, ?, ?, ?, ?)`)
      .run(trade.id, req.userId, rateeId, stars, review ?? null);
    res.status(201).json(prepare(`SELECT * FROM ratings WHERE id = ?`).get(info.lastInsertRowid));
  } catch (err) {
